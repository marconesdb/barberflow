...
export default function Perfil() {
  ...
  const handleSalvarDados = async () => {
    ...
    setSalvando(true);
    try {
      const res = await api.patch('/auth/perfil', dados);
          ...
          <div className="grid md:grid-cols-2 gap-6">
            ...
            <div className="space-y-2">
              ...
              {editando ? (
                ...
                <p className="px-4 py-3 bg-zinc-50 rounded-2xl font-medium text-zinc-900">{(user as any)?.telefone || '—'}</p>
              )}
            </div>
            ...
            <div className="space-y-2">
              ...
              {editando ? (
                ...
                <p className="px-4 py-3 bg-zinc-50 rounded-2xl font-medium text-zinc-900">{(user as any)?.endereco || '—'}</p>
              )}
            </div>
          </div>

          {editando && (
            <p className="text-xs text-zinc-400">* Campo obrigatório. O e-mail não pode ser alterado.</p>
          )}
        </motion.div>
      )}
